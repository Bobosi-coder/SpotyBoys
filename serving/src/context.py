"""
Serving Context — loads real artifacts or generates mock data.
Handles embedding lookups, candidate generation, cross-feature computation.
"""
import numpy as np
...
class ServingContext:
    ...
    def _load_real(self, d):
        ...
        centroids_path = os.path.join(d, "user_centroids.pkl")
        if os.path.exists(centroids_path):
            with open(centroids_path, "rb") as f:
                raw = pickle.load(f)
                self.user_centroids = {}
                for uid, clist in raw.items():
                    self.user_centroids[int(uid)] = [
                        np.array(c[0] if isinstance(c, (list, tuple)) else c, dtype=np.float32)
                        for c in clist
                    ]
        else:
            self.user_centroids = {}
        print(f"[context] Catalog: {len(self.track_ids)}, Users: {len(self.user_centroids)}")

    def _init_mock(self, n):
        print(f"[context] Mock mode (catalog={n})")
    ...
    def get_embeddings(self, track_ids):
        ...
        for tid in track_ids:
            row = self.track_to_row.get(tid)
            out.append(self.embeddings[row] if row is not None else np.zeros(self.d_emb, dtype=np.float32))
        return np.array(out, dtype=np.float32)

    def get_user_centroid(self, user_id, session_mean):
        if user_id in self.user_centroids:
            cs = self.user_centroids[user_id]
            return min(cs, key=lambda c: float(np.linalg.norm(c - session_mean))).astype(np.float32)
    ...
    def get_candidates(self, n=200, exclude=None):
        ...
        if len(pool) > n:
            ...
            cand = pool[:n]
        return cand, self.get_embeddings(cand)
